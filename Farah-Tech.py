...
print " %sFB%s Group%s Hack %s9-07-2021%s (0:55)%s" % (W, B, R, C, Y, N)

print " %sCreated by: %sLooy Farahi001 %s$$$%s Ehsan Amparator%s" % (W, R, W, Y, N)
print "                      %sAnd %sFarah Hack Team%s" % (W, R, N)

print " %sTelegram: %sFarah_Hack%s" % (W, C, N)

print " %sFacebook: %shttps://Farah_Hack.net%s" % (W, B, N)

print " %sFarahi and Amparator is your Father%s" % (C, N)
...
print " %sLanguage/select:%s" % (W, N)

print

print "%s (%s01%s)%s -- %sPashto%s" % (B, C, B, Y, W, N)

print "%s (%s02%s)%s -- %sDari%s" % (B, C, B, Y, W, N)

print "%s (%s03%s)%s -- %sFarahi%s" % (B, C, B, Y, W, N)

print

bahasa = raw_input("%s[%s*%s] %sFarahi/Choose/Amparator:%s " % (C, Y, C, W, B))
